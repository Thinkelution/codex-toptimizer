...
- **Read only** supports research in ordinary folders as well as Git repositories. **Allow project edits** retains Codex’s repository check; select a Git project for editing tasks. Failed turns show bounded error details in the conversation. **Use prompt again** copies the request into the composer for review without running it.
